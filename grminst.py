...
color_table = "#FFFFFF"
color_rich = "[#00C8FF]"
sys.stdout.write('\x1b]2; CROPT XD\x07')
		...
		if rom in['']:
			...
			if kukis in['']:
				...
				try:
					...
					#self.cekCOK()
					prints(Panel(f'{P2} login berhasil, jalankan ulang tool nya ketik {M2}:{P2} - {H2}python cropt.py',width=70,padding=(0,4),style=f"green"));exit()
			...
			print (f"[{H}• {P}] Link Cookiedough {m}:{o} hhttps://chrome.google.com/webstore/detail/cookiedough/hacigcgfiefikmkmmmncaiaijoffndpl");jeda(3)
			os.system(" xdg-open https://youtu.be/VrGZdY5L19k");exit()
...
class instagram:
	...
	def logo(self):
		for i in external:
			...
			prints(Panel(f"{P2}selamat datang {H2}{nama} {P2}gunakan toolsnya",width=60,padding=(0,9),style=f"green"))
	...
	def menu(self):
		...
		if c in['',' ']:
			...
			gan = input (f"[{H}• {P}] anda ingin crack massal id? y/t : ")
			if gan in['']:
				exit (f'[{H}• {P}] isi yang benar');jeda(2)
			elif gan in['y','Y']:
			...
			try:m=int(input(f'[{H}• {P}] jumlah target %s> %s'))
		...
		try:
			for ba in range(100):
				...
				try:
					...
					sys.stdout.write (f'\r{pu}╰─{br} mengumpulkan user{me} >{hi} {str(len(internal))} ')
...
def key():
	os.system("clear") 
	banner()
	prints(Panel(f"{P2}silahkan masukan lisensi tools agar bisa masuk ke tools Insta GaramXD\njika anda belum mempunyai lisensi ketik {H2}beli {P2}untuk melihat harga lisensi",style=f"green"))
	key = input(f"{P}[{B}?{P}] masukan lisensi : {H}")
	if key in ["beli","Beli","BELI"]:beli_bang()
	try:ses = requests.Session();asu = ses.get("https://app.cryptolens.io/api/key/Activate?token=WyI0MTg4NjA0OCIsImREOENPZmxhWjhTWFJsVlh2L2xkazlVY1gyL3ZPN3FqOGl6ZUJFMTciXQ==&ProductId=19373&Key=%s&Sign=True"%(key)).json()['licenseKey']['key'];open("data/lisensi.txt","w").write(key);prints(Panel(f"{H2}selamat lisensi yang anda masukan terdaftar ke server Insta Garam XD",width=80,padding=(0,6),style=f"green"));time.sleep(3); __cek__().cekCOK()
	except KeyError:
		prints(Panel(f"{P2} lisensi yang anda masukan tidak terdaftar silahkan beli terlebih dahulu",width=80,padding=(0,1),style=f"green"));os.system("xdg-open https://wa.me/+6283192869915?text=assalamualaikum+bang+mau+beli+lisensi+crack+Instagram");time.sleep(2);exit()

###----------[ CEK LISENSI ]---------- ###				
def cek():
	try:x=open("data/lisensi.txt","r").read()
	except FileNotFoundError:key()
	try:x = requests.get("https://app.cryptolens.io/api/key/Activate?token=WyI0MTg4NjA0OCIsImREOENPZmxhWjhTWFJsVlh2L2xkazlVY1gyL3ZPN3FqOGl6ZUJFMTciXQ==&ProductId=19373&Key=%s"%(x)).json()['licenseKey']['key']; __cek__().cekCOK()
	except KeyError:
		prints(Panel(f"{P2}lisensi kamu sudah kedaluwarsa silahkan beli lisensi ke admin",width=80,padding=(0,6),style=f"{color_table}"));os.system("rm -rf data/lisensi.txt");os.system("xdg-open https://wa.me/+6283192869915?text=assalamualaikum+bang+mau+beli+lisensi+crack+Instagram");time.sleep(2);exit()
...
def beli_bang():
    ...
    if zxc in [""]:print(f"{P}[{M}!{P}] pilih yang bener broo jangan kosong");time.sleep(3);exit()
    elif zxc in ["1","01"]:jalan(f"{P}[{M}!{P}] anda akan di arahkan ke WhatsApp");os.system("xdg-open https://wa.me/+6283192869915?text=assalamualaikum+bang+mau+beli+lisensi+1+minggu");time.sleep(2);exit()
    elif zxc in ["2","02"]:jalan(f"{P}[{M}!{P}] anda akan di arahkan ke WhatsApp");os.system("xdg-open https://wa.me/+6283192869915?text=assalamualaikum+bang+mau+beli+lisensi+1+bulan");time.sleep(2);exit()
    elif zxc in ["3","03"]:jalan(f"{P}[{M}!{P}] anda akan di arahkan ke WhatsApp");os.system("xdg-open https://wa.me/+6283192869915?text=assalamualaikum+bang+mau+beli+lisensi+full+source");time.sleep(2);exit()
